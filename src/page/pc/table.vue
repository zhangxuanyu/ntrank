...
<div>
    <ul>
        <li class="table_header">
            <div>#</div>
            <div  v-if="lanfalgz" v-for="(item,index) in market_header" @click="sorting(index)" :data-flag='item.flag' @mouseenter="enter(index)" @mouseleave="leave(index)"  :style="item.span_style">
                <span>{{item.zh_name}}</span>           
                <img :src="arr[item.flag-1]" alt="" :style="item.img_style">
            </div>
            <div  v-if="!lanfalgz" v-for="(it,idx) in market_header" @click="sorting(idx)"  :data-flag='it.flag' @mouseenter="enter(idx)" @mouseleave="leave(idx)"  :style="it.span_style">
                <span>{{it.en_name}}</span>           
                <img :src="arr[it.flag-1]" alt=""  :style="it.img_style">
            </div>
        </li>
        <li class="table_li" v-for="(item,index) in list" v-if="index <= contrl && lanfalgz">
            <div>{{index+1}}</div>
            <div>
    ...
    <div class="checkall" v-if="list.length > 20" @click="showall">
        <span v-if="lanfalgz && show">查看全部</span>
        <span v-if="!lanfalgz && show">View All</span>
    </div>
</div>
</template>
<script>
...
export default {
    data(){
        return{
            ...
            market_header:[
                {},
                {zh_name:'交易市场',en_name:'Markets'},
                {zh_name:'交易对(USD)',en_name:'Pair'},
    ...
    created(){
        ...
        $.ajax({
        type:'post',
        url:'https://blockchain.ldzldq.com/tokenrank/tokenMarket.json',
        dataType: 'json',
        data:{
            code:num2
        },
        ...
        rank(num,index){
            ...
            if(1 == num ){
                ...
                this.list.sort(function(a,b){
	                return  parseFloat(a[string])-parseFloat(b[string])
                })
            }else{
                //从大到小
                 this.list.sort(function(a,b){
	                return  parseFloat(b[string])-parseFloat(a[string])
                })
            }
            this.list = this.list
